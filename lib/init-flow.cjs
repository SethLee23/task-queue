 ...
 * 无条件 git init——「从零新建」语义即独立新仓库；若父目录在某仓库内会产生嵌套仓库，由调用方/前端提示。
 ...
 * initCore 抛错时 create 模式已建的目录不清理（重试 attach 可幂等补完）。
 *
 * @param {{ mode: 'attach'|'create', root: string, gitInit?: boolean, answers: object }} opts
 * @param {string} opts.root root 必须是已经过 resolveInitPath 规范化的绝对路径（调用方负责，本函数不重复做危险目标守卫）
...
async function runInit({ mode, root, gitInit = false, answers }) {
  if (!path.isAbsolute(root)) throw new Error(`runInit 需要绝对路径: ${root}`);
  if (mode === 'create') {
 ...
 * 前置条件：root 目录必须已存在 .tasks/project.config.js（即此前已走过接入流程）。
 * @param {string} root root 必须是已经过 resolveInitPath 规范化的绝对路径（调用方负责，本函数不重复做危险目标守卫）
 * @returns {{ slug: string, root: string }}
 */
function registerOnly(root) {
  validateAttachRoot(root);
  if (!fs.existsSync(path.join(root, '.tasks', 'project.config.js'))) {
    throw new Error('该目录未接入过任务队列（缺 .tasks/project.config.js），请走「接入已有」流程');
  }
